/**
 * @file Contains DownloadManager definitions
 */

#include "download_manager.hpp"

DownloadManager::DownloadManager(HostInfo info) : m_host_info(info) {}
...
void DownloadManager::GetFile(std::string_view filename) {
    ...
    const auto results = resolver.resolve(info.host, info.service, ec);
    ...
    http::request<http::string_body> req{
        http::verb::get, std::string(info.endpoint) + std::string(filename),
        HTTP_VERSION};
    req.set(http::field::host, info.host.data());
